...
export class Pet implements IPet {
  ...
  init() {
    ...
    if(!this.affinity) this.affinity = sample(Object.values(PetAffinity).filter(x => x !== PetAffinity.None));
    ...
    allStats.forEach(stat => {
      ...
      Object.keys(this.$player.buffWatches).forEach(buffKey => {
        this.$player.buffWatches[buffKey].forEach((buff: IBuff) => {
          if(!buff.stats || !buff.stats[stat] || !buff.booster) return;
  ...
  public doUpgrade(upgrade: PetUpgrade) {
    ...
    if(upgrade === PetUpgrade.GatherTime && this.gatherTick && this.gatherTick.valueOf() > (Date.now() + upgradeGatherTime)) {
      this.updateGatherTick();
    }
  }
