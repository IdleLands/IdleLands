...
export class AncientWolf extends Achievement {

  static readonly statWatches = ['Game/IdleLands3/Wolfmaster'];
  static readonly type = AchievementType.Special;

  static descriptionForTier(tier: number): string {
    const baseStr = `Gain a title and +10% to all special stats for achieving a very needlessly tough challenge! 🍻`;
  ...
  static rewardsForTier(tier: number): any[] {
    const baseRewards: any[] = [
      { type: AchievementRewardType.StatMultipliers, stats: {
       ...
       { type: AchievementRewardType.Title, title: 'Wolf Tamer 🐺' }
    ];
